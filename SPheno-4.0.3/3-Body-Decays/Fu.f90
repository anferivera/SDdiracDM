...
& MS22,mP2,vvSM,vS,gThh,gTVWp,gTVZ,gFuFucFdFd,gFuFdcFeFv,gFuFdcFxeFxv,gFuFucFeFe,        & 
& gFuFucFuFu,gFuFucFxvFxv,gFuFucFvFv,gFuFucFxeFxe,epsI,deltaM,CheckRealStates,           & 
& gT,gPartial,BR)
...
Real(dp),Intent(inout) :: gFuFucFdFd(3,3,3,3),gFuFdcFeFv(3,3,3,3),gFuFdcFxeFxv(3,3,1,2),gFuFucFeFe(3,3,3,3),    & 
& gFuFucFuFu(3,3,3,3),gFuFucFxvFxv(3,3,2,2),gFuFucFvFv(3,3,3,3),gFuFucFxeFxe(3,3,1,1)

Real(dp),Intent(in) :: gThh(2),gTVWp,gTVZ

Real(dp) :: gFuFucFdFdi(3,3,3),gFuFdcFeFvi(3,3,3),gFuFdcFxeFxvi(3,1,2),gFuFucFeFei(3,3,3),        & 
& gFuFucFuFui(3,3,3),gFuFucFxvFxvi(3,2,2),gFuFucFvFvi(3,3,3),gFuFucFxeFxei(3,1,1)
...
gFuFdcFxeFxvi = 0._dp 
Call FuToFdcFxeFxv(i_run,MFd,MFxe,MFxv,MVWp,MFu,cplcFuFdVWpL,cplcFuFdVWpR,            & 
& cplcFxeFxvcVWpL,cplcFxeFxvcVWpR,IntegralVs,IntegralVVss,NVs,NVVss,gTVWptemp,           & 
& deltaM,epsI,check,gFuFdcFxeFxvi)

gFuFdcFxeFxv(i_run,:,:,:) = gFuFdcFxeFxvi 
gT(i_run) = gT(i_run) + Sum(gFuFdcFxeFxvi) 
...
gFuFucFxvFxvi = 0._dp 
Call FuToFucFxvFxv(i_run,MFu,MFxv,MVZ,Mhh,cplcFuFuhhL,cplcFuFuhhR,cplcFuFuVZL,        & 
& cplcFuFuVZR,cplcFxvFxvhhL,cplcFxvFxvhhR,cplcFxvFxvVZL,cplcFxvFxvVZR,IntegralSs,        & 
& IntegralSSss,IntegralVs,IntegralVSss,IntegralVVss,NSs,NSSss,NVs,NVSss,NVVss,           & 
& gTVZtemp,gThhtemp,deltaM,epsI,check,gFuFucFxvFxvi)

gFuFucFxvFxv(i_run,:,:,:) = gFuFucFxvFxvi 
gT(i_run) = gT(i_run) + Sum(gFuFucFxvFxvi) 
...
gFuFucFxeFxei = 0._dp 
Call FuToFucFxeFxe(i_run,MFu,MFxe,MVZ,cplcFuFuVZL,cplcFuFuVZR,cplcFxeFxeVZL,          & 
& cplcFxeFxeVZR,IntegralVs,IntegralVVss,NVs,NVVss,gTVZtemp,deltaM,epsI,check,            & 
& gFuFucFxeFxei)

gFuFucFxeFxe(i_run,:,:,:) = gFuFucFxeFxei 
gT(i_run) = gT(i_run) + Sum(gFuFucFxeFxei) 
...
Do gt1=1,3
  Do gt2=1,1
    Do gt3=1,2
gPartial(i1,n_length)= gFuFdcFxeFxv(i1,gt1,gt2,gt3)
n_length=n_length+1
     End Do 
  End Do 
End Do 
...
Do gt1=1,3
  Do gt2=1,2
    Do gt3=1,2
gPartial(i1,n_length)= gFuFucFxvFxv(i1,gt1,gt2,gt3)
n_length=n_length+1
     End Do 
  End Do 
End Do 
...
Do gt1=1,3
  Do gt2=1,1
    Do gt3=1,1
gPartial(i1,n_length)= gFuFucFxeFxe(i1,gt1,gt2,gt3)
n_length=n_length+1
     End Do 
  End Do 
End Do 
...
Real(dp),Intent(in) :: MFu(3),MFd(3),MVZ,MVWp,Mhh(2)
...
Subroutine FuToFdcFxeFxv(iIN,MFd,MFxe,MFxv,MVWp,MFu,cplcFuFdVWpL,cplcFuFdVWpR,        & 
& cplcFxeFxvcVWpL,cplcFxeFxvcVWpR,IntegralVs,IntegralVVss,NVs,NVVss,gTVWp,               & 
& deltaM,epsI,check,g,WriteContributions)

Implicit None 
 
Real(dp),Intent(in) :: MFd(3),MFxe,MFxv(2),MVWp,MFu(3)

Complex(dp),Intent(in) :: cplcFuFdVWpL(3,3),cplcFuFdVWpR(3,3),cplcFxeFxvcVWpL(2),cplcFxeFxvcVWpR(2)
...
If(Abs(MFu(iIN)).gt.(Abs(MFxv(gt3))+Abs(MFxe)+Abs(MFd(gt1)))) Then 
!-------------- 
!  VWp 
!-------------- 
Isum = Isum + 1 
...
coup(2) = Conjg(cplcFuFdVWpL(iIN,gt1)) 
coup(1) = Conjg(cplcFuFdVWpR(iIN,gt1)) 
coup(4) = Conjg(cplcFxeFxvcVWpL(gt3)) 
coup(3) = Conjg(cplcFxeFxvcVWpR(gt3))
Call IntegrateGaugeSS(Boson2,mass,coup,deltaM,epsI,IntegralVs,NVs,resR, check) 
resR= 1*resR ! color factor 
resS = resS + resR 
...
If (resD.ne.resD) Then 
Write(*,*) "NaN appearing in the following diagrams: " 
Write(*,*) "Fu->Fd cFxe Fxv Propagator: VWp" 
Write(*,*)  "M_in: ",m_in 
Write(*,*)  "mass: ",mass 
Write(*,*)  "coup: ",coup 
...
Real(dp),Intent(in) :: MFu(3),MFe(3),MVZ,Mhh(2)
...
Real(dp),Intent(in) :: MFu(3),MVZ,Mhh(2)
...
Subroutine FuToFucFxvFxv(iIN,MFu,MFxv,MVZ,Mhh,cplcFuFuhhL,cplcFuFuhhR,cplcFuFuVZL,    & 
& cplcFuFuVZR,cplcFxvFxvhhL,cplcFxvFxvhhR,cplcFxvFxvVZL,cplcFxvFxvVZR,IntegralSs,        & 
& IntegralSSss,IntegralVs,IntegralVSss,IntegralVVss,NSs,NSSss,NVs,NVSss,NVVss,           & 
...
Real(dp),Intent(in) :: MFu(3),MFxv(2),MVZ,Mhh(2)
...
If(Abs(MFu(iIN)).gt.(Abs(MFxv(gt3))+Abs(MFxv(gt2))+Abs(MFu(gt1)))) Then 
!-------------- 
!  VZ 
!-------------- 
Isum = Isum + 1 
...
If (resD.ne.resD) Then 
Write(*,*) "NaN appearing in the following diagrams: " 
Write(*,*) "Fu->Fu cFxv Fxv Propagator: VZ" 
Write(*,*)  "M_in: ",m_in 
Write(*,*)  "mass: ",mass 
Write(*,*)  "coup: ",coup 
gSum(gt1,gt2,gt3,Isum)= 0._dp
...
If (resD.ne.resD) Then 
Write(*,*) "NaN appearing in the following diagrams: " 
Write(*,*) "Fu->Fu cFxv Fxv Propagator: hh" 
Write(*,*)  "M_in: ",m_in 
Write(*,*)  "mass: ",mass 
Write(*,*)  "coup: ",coup 
gSum(gt1,gt2,gt3,Isum)= 0._dp
...
If (resS.ne.resS) Then 
Write(*,*) "NaN appearing in the following diagrams: " 
Write(*,*) "Fu->Fu cFxv Fxv Propagator: VZ,hh" 
Write(*,*)  "M_in: ",m_in 
Write(*,*)  "mass: ",mass 
Write(*,*)  "coup: ",coup 
gSum(gt1,gt2,gt3,Isum)= 0._dp  
...
If (resS.ne.resS) Then 
Write(*,*) "NaN appearing in the following diagrams: " 
Write(*,*) "Fu->Fu cFxv Fxv Propagator: hh,hh" 
Write(*,*)  "M_in: ",m_in 
Write(*,*)  "mass: ",mass 
Write(*,*)  "coup: ",coup 
gSum(gt1,gt2,gt3,Isum)= 0._dp  
...
Subroutine FuToFucFxeFxe(iIN,MFu,MFxe,MVZ,cplcFuFuVZL,cplcFuFuVZR,cplcFxeFxeVZL,      & 
& cplcFxeFxeVZR,IntegralVs,IntegralVVss,NVs,NVVss,gTVZ,deltaM,epsI,check,g,              & 
& WriteContributions)

Implicit None 
 
Real(dp),Intent(in) :: MFu(3),MFxe,MVZ

Complex(dp),Intent(in) :: cplcFuFuVZL(3,3),cplcFuFuVZR(3,3),cplcFxeFxeVZL,cplcFxeFxeVZR
...
If(Abs(MFu(iIN)).gt.(Abs(MFxe)+Abs(MFxe)+Abs(MFu(gt1)))) Then 
!-------------- 
!  VZ 
!-------------- 
Isum = Isum + 1 
...
If (resD.ne.resD) Then 
Write(*,*) "NaN appearing in the following diagrams: " 
Write(*,*) "Fu->Fu cFxe Fxe Propagator: VZ" 
Write(*,*)  "M_in: ",m_in 
Write(*,*)  "mass: ",mass 
Write(*,*)  "coup: ",coup 
...
End If
   End Do 
  g = oo512pi3 / Abs(MFu(iIN))**3*g
End Subroutine FuToFucFxeFxe 
 
 
End Module Fu3Decays_SDdiracDM 
