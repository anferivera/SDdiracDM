...
for n in range(0,Num):
    ...
    MDF = np.exp(np.random.uniform(np.log(10**(2)),np.log(1.*10**(4))))
    ...
    a21 = np.exp(np.random.uniform(np.log(10**(-6)),np.log(10**(0))))*signo()
    a22 = np.exp(np.random.uniform(np.log(10**(-6)),np.log(10**(0))))*signo()
    a31 = np.exp(np.random.uniform(np.log(10**(-6)),np.log(10**(0))))*signo()    
    a32 = np.exp(np.random.uniform(np.log(10**(-6)),np.log(10**(0))))*signo()
    ...
    # Cut, perturbativity in bij
    if np.abs(b11) > 3. or np.abs(b21) > 3.:
        continue
    if np.abs(b31) > 3. or np.abs(b12) > 3.:
        continue
    if np.abs(b22) > 3. or np.abs(b32) > 3.:
        continue 
